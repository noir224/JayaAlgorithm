...
/*
    for (int j = 0; j < MATRIX_D; j++)
    {
        ...
        r1[j] = ((float)rand() / RAND_MAX) * (MAXR_VALUE - MINR_VALUE) + MINR_VALUE;

        r2[j] = ((float)rand() / RAND_MAX) * (MAXR_VALUE - MINR_VALUE) + MINR_VALUE;
    }
   */ 
...
void *compareBestAndWorst(float bestfs[K], float bestxs[K][MATRIX_D], float worstfs[K], float worstxs[K][MATRIX_D])
{
    ...
    global_best_fit = bestf; //Best fitness of all fitnesses
    ...
    global_worst_fit = worstf; //Worst fitness of all fitnesses
...
int main()
{
    ...
    for (i = 0; i < MATRIX_P; i++) {
        for (j = 0; j < MATRIX_D; j++) {
...
*/
    ...
    int endRows[K]; //indecies of starting ending for each thread
    ...
    for (int n = 0; n < N; n++)
    {
        ...
        ///creating threads to continue Jaya algorithm
